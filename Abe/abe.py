...
<body>
    ...
    <p style="font-size: smaller">
        ...
        %(download)s
        Tips appreciated!
       <a href="%(dotdot)saddress/%(DONATIONS_SXC)s">SXC</a>
       <a href="%(dotdot)saddress/%(DONATIONS_LTC)s">LTC</a>
    </p>
	</div>
</body>
</html>
...
DEFAULT_LOG_FORMAT = "%(asctime)s [%(process)d:%(threadName)s] %(name)s %(levelname)s - %(message)s"
#DEFAULT_LOG_FILE = ""
...
class Abe:
    ...
    def __call__(abe, env, start_response):
        ...
        try:
            ...
            tvars['statpanel'] = page['statpanel']
    ...
    def get_default_chain(abe):
        return abe.chain_lookup_by_name('Sexcoin')
    ...
    def update_statpanel(abe, page, chain):
        ...
        rows = abe.store.selectall("""
            SELECT block_height, block_ntime,block_nbits from block where block_id = (SELECT chain_last_block_id from chain where chain_id=1)
            """)
        for row in rows:
    ...
    def q_nethash(abe, page, chain):
        ...
        #
        # todo: use view for this query (sexcoin_live_chain)
        
        if stop is None:
            stop = count
        num_digits=len(str(abs(interval)))
        count_use = int(str(count)[-num_digits:])
        offset = abs(count_use - interval)
        abe.log.info("query: %d,%d,%d,%d,%d,%d", num_digits, count_use, offset, start, stop, interval)
        rows = abe.store.selectall("""
            SELECT b.block_height, b.block_ntime, b.block_chain_work, b.block_nbits
            FROM sexcoin_live_chain b
            WHERE block_height >= ? AND block_height <= ? AND ((block_height - ?) % ? = 0) ORDER BY b.block_height""",
                (start,stop,offset,interval))
        #
        #
        if fmt == "csv":
            ret = NETHASH_HEADER
    ...
    def handle_richlist(abe, page):
        symbol = wsgiref.util.shift_path_info(page['env'])
        chain = abe.chain_lookup_by_name(symbol)
        page['chain'] = chain
        page['title'] = 'Sexcoin Richlist - Top 100'
        body = page['body'] 
        
        date=abe.store.selectrow("""
            SELECT configvar_value 
            FROM configvar
            WHERE configvar_name = 'richlist_date'""")
...
def main(argv):
    conf = {
        ...
        "template_vars": {
            ...
            "DONATIONS_LTC": DONATIONS_LTC,
            "DONATIONS_SXC": DONATIONS_SXC,
